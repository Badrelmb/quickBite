...
const CustomerLogin = () => {
  ...
  const handleLogin = async () => {
    ...
    if (loginError) {
      ...
      // ✅ Check sessionStorage for redirect destination from scanned QR
      const redirectRestaurant = sessionStorage.getItem('redirect_restaurant');
      const redirectTable = sessionStorage.getItem('redirect_table');

      if (redirectRestaurant && redirectTable) {
        // Clear values after use
        sessionStorage.removeItem('redirect_restaurant');
        sessionStorage.removeItem('redirect_table');

        navigate(`/client-order?restaurant_id=${redirectRestaurant}&table=${redirectTable}`);
      } else {
        // Fallback to redirectTo query param or dashboard
